...
#   1. Programs the FPGA once via JTAG
#   2. Connects picocom, resets, waits for BusyBox Linux to boot
#   3. Runs /Test/run_inference_benchmarks.sh on the SoC
#      (ImageNet CNN + Transformer suites)
#   4. Results saved to /Test/results/<timestamp>_<bitstream_name>/ on the SD card
#
# Usage:
...
EXPECT_SCRIPT="$SCRIPT_DIR/run_soc_inference.expect"
...
echo "=========================================="
echo "  Batch Inference Benchmark Runner"
echo "  Batch dir : $BATCH_DIR"
echo "  Bitstreams: $TOTAL"
echo "  Suites    : ImageNet CNN + Transformer"
echo "=========================================="
...
for bitfile in "${BITSTREAMS[@]}"; do
    ...
    # Step 1: Program FPGA once via Vivado
    echo "[1/2] Programming FPGA..."
    ...
    # Step 2: UART automation — boot + run inference benchmarks
    echo "[2/2] Waiting for Linux boot + running inference benchmarks..."
    if ! expect "$EXPECT_SCRIPT" "$bitname" "$bitfile"; then
        echo "ERROR: Inference benchmark step failed for $bitname — skipping."
        FAILED+=("$bitname")
        continue
    fi
